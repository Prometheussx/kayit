...
import random
import time
from tool.oto_tel import TextVerifiedAPI
    ...
    try:
        while True:
            ...
            if any(element.count() > 0 for element in [error_element, error_element_two]):
                ...
                new_phone_number, href = get_random_phone_number()
                if new_phone_number:
                    ...
                    if g_element.count() > 0:
                        new_sms_code = get_sms_for_request(href)
                        if new_sms_code:
